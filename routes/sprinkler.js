...
const {
  addSprinklers,
  MapWeatherToWeatherCondition,
  mapCropType,
  mapRegionType,
  mapSoilType,
  mapTemperature,
  amountToTime,
} = require("../helpers/sprinkler");
const { scheduler, unSchedule } = require("../scheduler");
...
router.post("/water/one", async (req, res) => {
  ...
  if (sprinkler.amt && sprinkler.amt[0]) water = sprinkler.amt[0];
  console.log(water);
...
router.post("/water/two", async (req, res) => {
  ...
  let sprinkler = await Sprinkler.findOne({ _id: _id });
  let water = sprinkler.waterTime / 2;
  if (sprinkler.amt && sprinkler.amt[1]) water = sprinkler.amt[1];
  console.log("Sending", String(amountToTime(water * 2.5)));
  res.send(String(amountToTime(water * 2.5)));
});
router.post("/predictedWater", async (req, res) => {
  ...
  let sprinkler = await Sprinkler.findOne({ _id: _id });
  if (sprinkler.waterTime) res.send({ water: sprinkler.waterTime });
  else res.send({ water: 7 });
});
